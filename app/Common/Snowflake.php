...
class Snowflake
{
   ...
   public function __construct($data_center_id, $unix_id)
   {
       if ($data_center_id > $this->max_data_center) {
           throw new Exception(trans('admin.data_center').'：0-' . $this->max_data_center);
       }
       if ($unix_id > $this->max_unix) {
           throw new Exception(trans('admin.machine_number').'：0-' . $this->max_unix);
       }
   ...
   public function generateId()
   {
       ...
       if ($unix_time < $this->last_time) {
           throw new Exception(trans('admin.current_time'));
       }
       if ($unix_time == $this->last_time) {
           $serial = ++$this->serial;
